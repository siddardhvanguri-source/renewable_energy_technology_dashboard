...
const navItems = [
  { href: "/", label: "Live instrument", icon: Activity },
  { href: "/history", label: "Telemetry history", icon: Database },
  { href: "/scenarios", label: "Test scenarios", icon: Gauge },
  { href: "/settings", label: "Controller settings", icon: Settings2 },
];
    ...
    <div className="platform-shell">
      <aside className={`side-rail ${open ? "open" : ""}`}>
        <div className="rail-brand">
          ...
          <div><small>ESP32 / MPPT</small><strong>CONTROL PLATFORM</strong></div>
        ...
        <nav className="rail-nav" aria-label="Main navigation">
          <small className="rail-label">WORKSPACE</small>
        ...
        <div className="rail-foot">
          <div className="rail-context"><ShieldCheck size={15} /><span>SOFTWARE SAFETY ONLINE</span></div>
          <div className="rail-context"><Cable size={15} /><span>WEBSOCKET READY</span></div>
      ...
      <div className="platform-main">
        <header className="platform-topbar">
          ...
          <div className="topbar-context"><span className="eyebrow">ENGINEERING CONSOLE</span><strong>{location === "/" ? "LIVE INSTRUMENT" : navItems.find((item) => item.href === location)?.label.toUpperCase() ?? "CONTROL PLATFORM"}</strong></div>
          <div className="topbar-right"><span className="runtime-chip"><span className="status-dot live-dot" /> API ONLINE</span><span className="runtime-chip hide-mobile"><Sun size={14} /> NOMINAL SUNLIGHT</span><button className="topbar-icon" aria-label="Documentation"><BookOpen size={15} /></button></div>
        </header>
