...
export default function Home() {
  ...
  const [notice, setNotice] = useState("Database-backed demo telemetry active");
  ...
  return <div className="instrument-page"><FlowField density="sparse" /><div className="ambient-orbit orbit-one" /><div className="ambient-orbit orbit-two" />
    <section className="instrument-heading"><div><div className="eyebrow">LIVE TELEMETRY / ARRAY A</div><h1>MPPT Tracking Instrument</h1><p>Review console · data persists to the controller history database · {snapshot?.device?.firmware ?? "firmware v2.4.1"}</p></div><div className="heading-actions"><span className={`status-pill ${connection.toLowerCase().replace(" / ", "-")}`}><span className="status-dot" />{connection}</span><button className="button subtle" onClick={connectEsp32}><Cable size={14} /> connect ESP32</button><button className="button icon" aria-label="Refresh snapshot" onClick={() => snapshotQuery.refetch()}><RefreshCw size={15} /></button></div></section>
    <section className="hero-grid"><LiquidGlassCard className="hero-card"><div className="hero-card-top"><div><span className="eyebrow">PRIMARY TELEMETRY</span><h2>P_pv <span>power output</span></h2></div><div className="hero-chip"><span className="status-dot" /> MPPT TRACKING</div></div><div className="hero-readout"><strong>{current.pPv.toFixed(1)}</strong><span>W<small>instantaneous</small></span><span className={`hero-delta ${powerDelta >= 0 ? "positive" : "negative"}`}>{powerDelta >= 0 ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />} {Math.abs(powerDelta).toFixed(1)} W <small>Δ / sample</small></span></div><div className="hero-sparkline"><Sparkline values={trend} /><div><span>-12 s</span><span>now</span></div></div><div className="hero-meta"><div><small>TRACKING ALGORITHM</small><strong>Extended Kalman Filter</strong></div><div><small>OPERATING POINT</small><strong>V_mp / I_ph lock</strong></div><div><small>SAMPLE RATE</small><strong>{snapshot?.settings?.sampleRateHz ?? 20} Hz / {snapshot?.settings?.isrPeriodUs ?? 10} μs ISR</strong></div></div></LiquidGlassCard>
      <LiquidGlassCard className={`stop-card ${stopped ? "stopped" : ""}`}><div className="stop-card-head"><div><span className="eyebrow">SAFETY CONTROL</span><h2>{stopped ? "Software stop" : "Controller armed"}</h2></div><ShieldCheck size={18} /></div><div className="stop-state"><span className={`status-dot ${stopped ? "red" : ""}`} /><strong>{stopped ? "OUTPUT DISABLED" : "OUTPUT ENABLED"}</strong></div><button className="stop-button" onClick={stopped ? resumeDemo : sendStop}>{stopped ? <Play size={18} /> : <CircleStop size={18} />}<span>{stopped ? "RESUME CONTROLLER" : "SOFTWARE STOP"}</span></button><p>{stopped ? "Resume is queued for the controller. Demo output is paused locally." : "Queues { cmd: \"stop\" } and stores an auditable command record."}</p></LiquidGlassCard>
    </section>
    ...
    <div className="notice-bar"><Terminal size={13} /><span>{notice}</span><span className="notice-spacer" /><span className="micro-label">TRANSPORT: {connection === "DEMO / MOCK" ? "DB + SYNTHETIC" : "WEBSOCKET"}</span><span className="status-dot" /></div>
