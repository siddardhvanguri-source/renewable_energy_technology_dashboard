...
export default function Home() {
  ...
  const [dataSource, setDataSource] = useState<"DEMO" | "SIMULATION" | "HARDWARE">("DEMO");
  const socketRef = useRef<WebSocket | null>(null);
  const snapshot = snapshotQuery.data;
  const simulationStatusQuery = trpc.simulation.status.useQuery();
  const simulationRowsQuery = trpc.simulation.latest.useQuery({ limit: 60 }, { enabled: dataSource === "SIMULATION" });
  ...
  useEffect(() => { if (stopped || dataSource !== "DEMO") return; const timer = window.setInterval(() => setTelemetry((items) => [...items.slice(-59), demoFrame()]), 1100); return () => window.clearInterval(timer); }, [stopped, dataSource]);
  useEffect(() => { if (dataSource !== "SIMULATION" || !simulationRowsQuery.data?.length) return; setTelemetry(simulationRowsQuery.data.map((row) => ({ timestampMs: row.timestampMs, vPv: row.vPv, iPv: row.iPv, pPv: row.pPv, vMp: row.vMp, iPh: row.iPh, duty: row.duty, efficiency: row.efficiency }))); setConnection("LIVE"); setNotice(`MATLAB simulation loaded · ${simulationRowsQuery.data.length} samples`); }, [dataSource, simulationRowsQuery.data]);
  ...
  const changeSource = (source: "DEMO" | "SIMULATION" | "HARDWARE") => { setDataSource(source); if (source === "DEMO") { socketRef.current?.close(); setConnection("DEMO / MOCK"); setNotice("Demo data is running"); } else if (source === "SIMULATION") { socketRef.current?.close(); setConnection(simulationStatusQuery.data?.available ? "LIVE" : "CONNECTING"); setNotice(simulationStatusQuery.data?.available ? "MATLAB simulation ready" : "Waiting for simulation/output/simulation_results.csv"); } else { connectEsp32(); } };
  ...
  return <div className="instrument-page"><FlowField density="sparse" /><div className="ambient-orbit orbit-one" /><div className="ambient-orbit orbit-two" />
    <section className="instrument-heading"><div><div className="eyebrow">ARRAY A</div><h1>MPPT Dashboard</h1><p>Power, voltage, and current at a glance.</p></div><div className="heading-actions"><DataSourceSelector source={dataSource} onChange={changeSource} /><span className={`status-pill ${connection.toLowerCase().replace(" / ", "-")}`}><span className="status-dot" />{connection}</span><button className="button subtle" onClick={connectEsp32}><Cable size={14} /> connect</button><button className="button icon" aria-label="Refresh" onClick={() => snapshotQuery.refetch()}><RefreshCw size={15} /></button></div></section>
    ...
    <div className="notice-bar"><Terminal size={13} /><span>{notice}</span><span className="notice-spacer" /><span className="micro-label">{dataSource === "DEMO" ? "DEMO" : dataSource === "SIMULATION" ? "MATLAB" : "LIVE"}</span><span className="status-dot" /></div>
    <section className="chart-section"><div className="section-label">TRACKING ANALYSIS / S5 <i /></div><div className="chart-grid primary"><EkfPoChart /><RippleChart /></div><div className="chart-grid secondary"><PvCurveChart /><EfficiencyChart efficiency={current.efficiency} /><DutyChart duty={current.duty} /></div><div className="section-label">REAL-TIME / CONTROLLER RESPONSE <i /></div><div className="chart-grid live"><LiveTelemetryChart telemetry={telemetry} /><ResponseBandChart /></div></section>
    ...
    {notificationsOpen ? <div className="notification-drawer"><div className="drawer-head"><span>EVENT LOG</span><button onClick={() => setNotificationsOpen(false)} aria-label="Close event log"><X size={14} /></button></div><div className="event-item"><span className="status-dot" /><div><strong>MPPT stable</strong><small>EKF tracking within target band</small></div><time>now</time></div><div className="event-item"><span className="status-dot amber" /><div><strong>History service</strong><small>{snapshot?.history?.length ?? 0} persisted samples loaded</small></div><time>live</time></div></div> : null}<button className="floating-events" aria-label="Open event log" onClick={() => setNotificationsOpen((value) => !value)}><Bell size={16} /><span>2</span></button>
  </div>;
}
